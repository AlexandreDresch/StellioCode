  ...
  const { getRootProps, isDragActive } = useDropzone({
    ...
    onDrop: (acceptedFiles) => {
      if (acceptedFiles.length > 0) {
        handleFilesChange([...files, ...acceptedFiles]);
      }
    },
    ...
    <div className="w-full" {...getRootProps()}>
      ...
      >
        <input
          ...
          onChange={(e) => {
            if (e.target.files && e.target.files.length > 0) {
              handleFilesChange([...files, ...Array.from(e.target.files)]);
            }
          }}
          className="hidden"
        />
        ...
        <div className="justify-left flex flex-col items-start">
          <p className="relative z-20 text-sm font-semibold">Enviar Imagens</p>
          ...
          <div className="relative mx-auto mt-10 w-full max-w-xl">
            {files.length > 0 && (
              <motion.div
                ...
                className={cn(
                  "relative z-40 mx-auto mt-4 flex w-full flex-col items-start justify-start gap-4 overflow-hidden rounded-md bg-white p-4 dark:bg-neutral-900",
                  "shadow-sm",
                )}
              >
                {files.map((file, index) => (
                  <div
                    key={index}
                    className="flex w-full flex-col items-start justify-between rounded-md bg-gray-50 p-3 dark:bg-neutral-800"
                  >
